    ...
    private void SpawnBalls( Labyrinth newLab, int max)
    {
        ...
        for (var d = -deep; d <= deep; d++)
        {
            for (var j = 0; j < sq*sq; j++)
            {
                ...
                var ball = Instantiate(ballSize == 2 ? gameConfig.ballBig : ballSize == 1 ? gameConfig.ballMid : gameConfig.ball[Random.Range(0, gameConfig.ball.Length)], newLab.transform);
