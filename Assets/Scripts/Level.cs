        ...
        for (var d = -deep; d <= deep; d++)
        {
            for (var j = 0; j < sq*sq; j++)
            {
                ...
                if (PlayerPrefs.GetInt("level") >= 1)
                    ballSmall = gameConfig.waterBall;
                else
                    ballSmall = gameConfig.ball[Random.Range(0, gameConfig.ball.Length)];

                if (ballSmall == null)
                    ballSmall = gameConfig.ball[0];
                //=====================================
                var ball = Instantiate(ballSize == 2 ? gameConfig.ballBig : ballSize == 1 ? gameConfig.ballMid : ballSmall, newLab.transform);
